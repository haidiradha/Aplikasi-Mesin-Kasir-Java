package Tampilan;

import java.awt.Container;
import java.awt.BorderLayout;
import java.awt.Color;
import java.awt.Dimension;
import java.awt.Font;
import java.awt.Toolkit;
import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.Vector;

import javax.swing.ImageIcon;
import javax.swing.JButton;
import javax.swing.JComboBox;
import javax.swing.JFrame;
import javax.swing.JOptionPane;
import javax.swing.JPanel;
import javax.swing.JScrollPane;
import javax.swing.JTable;
import javax.swing.JTextField;
import javax.swing.JLabel;
import javax.swing.table.DefaultTableModel;
import javax.swing.table.JTableHeader;
import javax.swing.JMenu;
import javax.swing.JMenuItem;
import javax.swing.JMenuBar;

import Database.Obat;
import Database.Transaksi;
import Setting.*;
import Tampilan.Listener.CustActionListener;

public class DBTransaksi extends JFrame {
	private Utama core;

	private Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
	private Vector<Obat> obat = new Vector<Obat>();
	private Vector<String> nmObat = new Vector<String>();

	private JTable tbl;

	public DBTransaksi(Utama core) {
		super("Data Transaksi");
		this.core = core;
		setResizable(false);

		setIconImage(Toolkit.getDefaultToolkit().getImage(DBTransaksi.class.getResource("/Gambar/okboy.png")));
		setSize(500, 300);
		setLocation((screenSize.width - getWidth()) / 2,
				(screenSize.height - getHeight()) / 2);
		getContentPane().setLayout(null);
		Container container = this.getContentPane();
		container.setBackground(new Color(0, 255, 102));
		tbl = new JTable(Syntax.resultSetToTableModel(Syntax
				.getListTransaksi(core.getConnection())));
		Syntax.disableTableEdit(tbl);
		JPanel panTbl = new JPanel();
		panTbl.setLayout(new BorderLayout());
		panTbl.add(new JScrollPane(tbl), BorderLayout.CENTER);
		panTbl.setBounds(5, 5, 485, 195);
		panTbl.setBackground(Color.WHITE);
		
		JButton buttonDelete = new JButton("Hapus Transaksi");
		buttonDelete.setFont(new Font("Comic Sans MS", Font.PLAIN, 14));
		buttonDelete.addActionListener(new CustActionListener(core, this, tbl,
				buttonDelete, CustActionListener.HAPUS_TRANS));
		buttonDelete.setBounds(120, 211, 250, 50);
		container.add(buttonDelete);
		
		container.add(panTbl);
		
		JLabel Backg = new JLabel();
		Backg.setBounds(0, 0, 800, 400);
		getContentPane().add(Backg);
		
		ImageIcon back = new ImageIcon(DBTransaksi.class.getResource("/Gambar/back.jpg"));
		Backg.setIcon(back);
	}

	public Transaksi getTransaksi() {
		if (tbl.getSelectedRow() >= 0) {
			String val = tbl.getValueAt(tbl.getSelectedRow(), 0).toString();
			
			return new Transaksi(Integer.parseInt(val));
		} else {
			return null;
		}
	}

	public void resetForm() {
		if (tbl.getSelectedRow() >= 0) {
			((DefaultTableModel) tbl.getModel())
					.removeRow(tbl.getSelectedRow());
		}
	}
}
